...
- [ ] T003 Crear `.editorconfig` en la raíz del repositorio con las reglas del proyecto (ver plan.md §Configuración .editorconfig): `no-wildcard-imports` deshabilitado, `indent_size = 4`, `max_line_length = 120`
- [ ] T004 Ejecutar `./gradlew ktlintFormat` para auto-corregir todas las violaciones en el código existente; verificar con `./gradlew ktlintCheck` que termina sin errores; hacer commit `style: aplicar formato ktlint al código existente`
...
- [ ] T010 Crear `.github/workflows/ci.yml` con dos jobs secuenciales: `lint` (corre `./gradlew ktlintCheck`) y `test` (corre `./gradlew :shared:jvmTest`); ambos en `ubuntu-latest` con JDK 21 temurin y `gradle/actions/setup-gradle@v4`; trigger en `push` a cualquier branch y `pull_request` hacia `main`
...
- [ ] T014 Añadir campo `val iconRes: DrawableResource` a cada `data object` en `Screen.kt` referenciando `Res.drawable.ic_nav_*`; añadir el import necesario (`dictionarymuchik.shared.generated.resources.*`)
- [ ] T015 Actualizar `MuchikNavigation.kt`: eliminar la función `getIconForScreen()` completa; en `MuchikNavigationRail` y `MuchikModalDrawer` reemplazar `imageVector = getIconForScreen(screen)` por `painter = painterResource(screen.iconRes)`; actualizar los imports
...
- `Screen.Sync` no aparece en `navItems` (no es un ítem de menú navegable) — no necesita `iconRes`.
- La propiedad `iconRes` en `Screen` puede declararse con valor `null` en `Screen.Sync` o simplemente no añadirse (depende de si quieres mantener la exhaustividad del sealed class).
